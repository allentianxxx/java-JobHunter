...
max-age 指令出现在**请求报文中**，并且缓存资源的缓存时间小于该指令指定的时间，那么就能接受该缓存。

max-age 指令出现在**响应报文中**，**表示缓存资源在缓存服务器中保存的时间**。

```html
Cache-Control: max-age=31536000
```

Expires 首部字段也可以用于**告知缓存服务器**该资源什么时候会过期。
...
**反向代理的作用**：集群式部署实现负载均衡、CDN技术、**前端服务器**、Nginx等。
...
HTTPS 并不是新协议，而是让 HTTP 先和 **SSL（Secure Sockets Layer）**通信，再由 SSL 和 TCP 通信，也就是说 **HTTPS 使用了隧道**进行通信。
